...
export default function OrdersPage() {
  ...
  const [selectedAreas] = useState<string[]>(filters.areas);
  const [selectedDate] = useState(filters.date);
  const [partnerMetrics, setPartnerMetrics] = useState<
    Record<string, PartnerMetrics>
  >({});
  ...
  const updateOrderStatus = async (
    ...
    try {
      const res = await fetch(`/api/orders/status?orderId=${orderId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ status: newStatus }), // Send only the status in the body
      });
